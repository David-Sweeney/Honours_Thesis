...
class zernikePSF:
    def __init__(self, radius = 1.0, wavelength = 1500e-9, pixscale = 0.01, FOV_pixels = 128):
        # wavelength defaults to 1500e-9
        self.psf = None
        self.wf = None
        self.radius = radius
    ...
    def saveToRSoft(self, outfile='PSFOut', size_data=400, verbose=2):
        """
    ...
    def makeMultiplePSFs(self, coeffsList, outpath='./', filePrefix='zernikePSFs', extraPlots=False, size_data=400,
                         makeBatFile=False, saveAllData=False, indFile='bptmp.ind', outPrefix='BPScan',
                         numBatfiles=1, trimLeadingCoeffnames=None, wlList=None, wlToPoppy=True, verbose=2):
                         # Verbose = 0 is silent
                         # Verbose = 1 will print every 10th PSF produced
                         # Verbose = 2 will print PSF info and writing to file
        allOutfiles = []
        allData = []
        ...
        for coeffs in coeffsList:
            print('Making PSF number:', count + 1)
            if verbose > 0:
                if verbose > 1:
            ...
            self.makeZernikePSF(coeffs=coeffs, show=False, extraPlots=extraPlots)
            ...
            #print(filestr)
            self.saveToRSoft(outfile=outpath+filestr, size_data=size_data, verbose=verbose)
        ...
        if makeBatFile:
            ...
            nf = len(allOutfiles)
            batfileLength = ceil(nf / numBatfiles)
            allBatfileNames = []
            ...
            for k in range(numBatfiles):
                ...
                for launch_file in curOutfiles:
                    if wlList is None:
                        ...
                        # cmdStr = '{progname} {indFile} prefix={outPrefix}{launch_file} launch_file={launch_file}_inputfield.fld wait=0\n'.format(progname=progname, indFile=indFile, \
                        #         outPrefix=outPrefix, launch_file=launch_file)
                    else:
            ...
            if numBatfiles > 1:
                ...
                for batfilename in allBatfileNames:
                    cmdStr = 'start /belownormal cmd /k call ' + batfilename +'\n'
