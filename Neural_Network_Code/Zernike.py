...
# # TODO:
    ...
    # augment_data ONLY works for 8 Zernikes

# List of changes for Athena
    # Zernike scaling 5 -> 2.5 in load_data
    # Radians in plot_model units
    # Zernike range

class ModelInfo:
    def __init__(self, datasets, model_name, suffix, model_layers=None, epochs=5000, model=None, transfer_datasets=None, learning_rate=0.001,
                    l2_rate=0, dropout=None, resnet=False, training_examples=None, test_size=0.10, val_size=0.50, augmenting_data=True):
        self.model = model
        self.input_data = ''
        self.output_data = ''
        ...
        self.zernikes = 0
        self.zernike_mean = None
        self.zernike_deviation = None
        self.zernike_max = None
        self.train_x = None
        self.train_y = None
        self.val_x = None
        self.val_y = None
    ...
    def normalise_zernikes(self, zernike_data):
        if self.datasets[0] == 'BigOne10':
            self.zernike_mean = np.mean(zernike_data, axis=0)
            self.zernike_deviation = np.std(zernike_data, axis=0)
            zernike_data = (zernike_data - self.zernike_mean)/self.zernike_deviation
        elif self.datasets[0] == 'PracticalData9':
            self.zernike_mean = np.mean(zernike_data, axis=0)
            self.zernike_deviation = np.std(zernike_data, axis=0)
            zernike_data = (zernike_data - self.zernike_mean)/self.zernike_deviation
            # zernike_data = zernike_data * 2.5
        elif self.datasets[0] == 'BigOne8' or self.datasets[0] == 'BigOne12':
            zernike_data = zernike_data * 2
        elif self.datasets[0] == 'BigOne40':
            zernike_data = zernike_data
        else:
            zernike_data = zernike_data / self.zernike_max
        return zernike_data

    def reverse_zernike_normalisation(self, zernike_data):
        if self.datasets[0] == 'BigOne10':
            zernike_data = zernike_data*self.zernike_deviation + self.zernike_mean
        elif self.datasets[0] == 'PracticalData9':
            zernike_data = zernike_data*self.zernike_deviation + self.zernike_mean
            # zernike_data = zernike_data / 2.5
        elif self.datasets[0] == 'BigOne8' or self.datasets[0] == 'BigOne12':
    ...
    def normalise_data(self, datasets, zernike_data, flux_data):
        zernike_data = self.normalise_zernikes(zernike_data)
    ...
    def load_data(self, load_specialised=False, original_dataset=None):
        # This function takes the datasets in datasets, reads them in and creates training and test sets based on the data
        # once it has been shuffled. This processess is repeatable since a random_state is used.
        ...
        zernike_data, flux_data = self.normalise_data(self.datasets, zernike_data, flux_data)
        ...
        # if self.suffix == '-f37':
        #     transfer_zernike_data = transfer_zernike_data[:1000]
        #     transfer_flux_data = transfer_flux_data[:1000]
        # else:
        #     print('PANIC!')
        #     input('')
        ...
        self.train_x, self.test_x, self.train_y, self.test_y = train_test_split(input_data, output_data, test_size=self.test_size, random_state=0)
        self.train_x, self.val_x, self.train_y, self.val_y = train_test_split(self.train_x, self.train_y, train_size=self.training_examples, test_size=self.val_size, random_state=0)
        if self.transfer_datasets is not None:
    ...
    def plot_training(self, model, history, test_x, test_y, predicted_y, filepath, show_plots):#, model_name='model', suffix=''):
        ...
        plt.text(0, 0, 'Model Name: {}\nZernike Modes: {}\nSuffix: {}\nDatasets: {}\nTransfer Datasets: {}\nTotal Dataset size: {}\nTraining Examples: {}\nModel Layers: {}\nActivation Function: {}\nLearning Rate: {}\nDropout: {}\nLambda (L2_rate): {}\nBatch Normalisation: {}\nEarly Stopping: {} epochs\nReduce LR on Plateau: {}\nBest Validation MSE: {}\n{} MAE: {}\n'.format(
                    self.model_name, self.zernikes, self.suffix, self.datasets, self.transfer_datasets, self.dataset_size, self.training_examples, layers, 'LeakyReLU', self.learning_rate,
                    self.dropout, self.l2_rate, 'On',  self.early_stopping_patience, 'On', loss, ' '*24, np.amin(history.history['val_mae'])))
    ...
    def run(self, show_plots=True, show_training_predictions=False, verbose=1, load=True):
        ...
        # Create callbacks
        early_stopping = EarlyStopping(monitor='val_loss', patience=self.early_stopping_patience, verbose=0, min_delta=1e-7, mode='min')
        ...
        reduce_lr_loss = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=40, verbose=1, min_delta=1e-7, mode='min')
    ...
    def test_n_data(self, model, set_x, set_y, num):
        ...
        # np.random.seed() # Actual random seed, so as not to only see the same num training examples
        random_choices = np.random.randint(0, len(self.test_x), size=num)
        # np.random.seed(1) # Revert seed
        # create new array of test_x and test_y with these elements
        new_set_x = np.array([set_x[i].copy() for i in random_choices])
        new_set_y = np.array([set_y[i].copy() for i in random_choices])
        predicted_y = model.predict(new_set_x)
        if len(predicted_y[0]) != 19:
            # We are then in a reverse model, predicting Zernikes
            # So we should undo the normalisation
            predicted_y = self.reverse_zernike_normalisation(predicted_y)
            new_set_y = self.reverse_zernike_normalisation(new_set_y)
...
def plot_best_model_test_predictions():
    ...
    # for model_full_name in ['Athena-t1',]:
            ...
            for i in range(len(suffixes)):
                ...
                if both:
                    ...
                    ModelInfo(
                        ...
                        learning_rate = default_learning_rate,
                        l2_rate = regularisations[i][0],
                        dropout = regularisations[i][1]).run(show_plots = 0, verbose = 0)
            ...
            for i in range(len(suffixes)):
                ...
                if both:
                    ...
                    ModelInfo(
                        ...
                        augmenting_data = True,
                        test_size = 0.0001,
                        val_size = 0.3).run(show_plots = 0, verbose = 0, show_training_predictions = False)
        else:
...
# two = [2.27, 22.0, 69.4, 80.3]
# five = [12.4, 48.6, 140]
# zernikes_two = [7, 11, 16, 19]
# zernikes_five = [8, 8, 11]
# plt.plot(zernikes_two, two)
# plt.plot(zernikes_five, five)
# plt.legend(['$\pm 0.2 \mu m$', '$\pm 0.5 \mu m$'])
# plt.show()
